...
Якщо ви не знайшли ендпоїнта з методами `POST`, `PATCH` або `PUT`, то можна надіслати `POST` запит на будь-який, навіть неіснуючий, ендпоїнт. Однак тут є важливий нюанс!

Nginx не зберігає велике тіло запиту в оперативній пам’яті. Натомість воно кешується у тимчасовий файл:

```
[warn] 33#33: *7963 a client request body is buffered to a temporary file /var/cache/nginx/client_temp/0000013805, client: 172.25.0.1, server: localhost, request: "POST /api/hit1 HTTP/1.1", host: "127.0.0.1"
```

Щоб тіло запиту залишалося в оперативній пам’яті, потрібно, щоб бекенд почав його обробляти. У такому випадку **Slow Loris** стає ще ефективнішим!
