...
- `/json` — ендпоїнт приймає **JSON** тіло для тестування навантаження на сервер під час парсингу.  
- `/thinking` - ендпоїнт довго "думає". 

### Налаштування середовища  
Це все працюватиме на **віртуальній машині** з гіпервізором **KVM**. Початково сервер матиме **1 CPU ядро**, і ми поступово збільшуватимемо кількість ядер, щоб оцінити, як це впливає на продуктивність. Атака буде проводитись з іншого фізичного пристрою в локальній мережі.

Спробуємо навантажити сервер готовими скриптами з **GitHub** та напишемо власні скрипти для тестування. Порівняємо продуктивність і зробимо висновки.  
...
### Відправляємо велику кількість запитів одночасно
Тут пишемо свій. Відправляємо їх одночасно за допомогою синхронізації по останньому байту